...
<template>
  <header class="z-10 bg-white py-4 shadow-md dark:bg-gray-800">
    <div class="flex h-full items-center justify-between px-6 text-purple-600 dark:text-purple-300">
      <!-- Mobile hamburger -->
      <button ref="ignoreElRef"
        class="-ml-1 mr-5 rounded-md p-1 focus:shadow-outline-purple focus:outline-none md:hidden" aria-label="Menu"
      ...
      <div class="flex flex-1 justify-center lg:mr-32">
        <div class="relative mr-6 w-full max-w-xl focus-within:text-purple-500">
          ...
          <input
            ...
            type="text" placeholder="Search for requests" aria-label="Search"
            @input="(val: Event) => requestStore.search(val as InputEvent)" />
        </div>
      </div>
      <ul class="flex flex-shrink-0 items-center space-x-6">
        <!-- Theme toggler -->
        <li class="flex">
          <button class="rounded-md focus:shadow-outline-purple focus:outline-none" aria-label="Toggle color mode"
        ...
        <!-- Profile menu -->
        <li class="relative">
          <button class="rounded-full align-middle focus:shadow-outline-purple focus:outline-none"
            aria-label="Profile and logout" aria-haspopup="true" @click="toggleProfileMenu">
            <div>
              <!-- icon here -->
          ...
          <template v-if="isProfileMenuOpen">
            <ul v-on-click-outside="toggleProfileMenu"
              ...
              <li class="flex">
                <a class="inline-flex w-full cursor-pointer items-center rounded-md px-2 py-1 text-sm font-semibold transition-colors duration-150 hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
